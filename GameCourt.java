...
public class GameCourt extends JPanel implements ActionListener{
	...
	//points scored based on how many lines cleared at one time
	private static final int[] lineScores = {0,59,113,331,1217};
	private int score = 0;
	//keeps track of what shape is where
	private int [][] board;
		...
		for(int i = 0; i<4; i++){
			int newX = x + shape.getX(i);
	      		...
	      		if (newX < 0 || newY < 0 || newX >= COURT_WIDTH || newY>= COURT_HEIGHT) {
	        		...
	        		//Don't move if shape intersects with another shape
				return false; 
	      		}
		}
	...
	private void newShape() {
		...
		// so that it looks the same as in preview
	    	curShape = curShape.flip();
	    	nextShape.setRandomShape();
	    	curX = 5;
	    	curY = COURT_HEIGHT - 1 + curShape.minY();
		
		// if cannot move anymore, game over
	    	if (!move(curShape, curX, curY)) {
	      		curShape.setShape(0);
	      		timer.stop();
	      		playing = false;
	      		repaint();
	...
	private void clearLines() {
			...
			// max speed reached at level 10
	      		timer.setDelay(550-(Math.min(level, 10)-1)*50);
	      		hasFallen = true;
	    	}
	}
	...
	private void shapeDrop() {
		...
		// remove lines so you see block before it disappears
	    	clearLines();
	    	repaint();
	    	score+=17;
	    	newShape();
	...
	public void paint(Graphics g) {
		...
		for (int i = 0; i < COURT_HEIGHT; i++) {
		  	for (int j = 0; j < COURT_WIDTH; j++) {
				// start from bottom of grid
		    		int t = board[COURT_HEIGHT - i - 1][j];
		    		if (t != 0) {
	...
	public void actionPerformed(ActionEvent e) {
		// if fallen to the ground, get next piece
	    	if (hasFallen) {
	      		hasFallen = false;
	    	} else {
	      		oneLineDown();
